...
export const addProduct = async (req, res) => {
    try {
        ...
        const { productName, productDescription, category, price, discountPrice, shelfTime, stock, expiryDate, manufactureDate, ingredients } = req.body;
        ...
        if (productWithImages?.images) {
            productImagesBase64 = await Promise.all(productWithImages.images.map(async (image) => {
                try {
...
export const fetchProducts = async (req, res) => {
    try {
        ...
        const updatedProducts = await Promise.all(products.map(async (p) => {
            const imagesBase64 = await Promise.all(p.images.map(async (image) => {
                try {
                    const imagePath = path.join(process.cwd(), "uploads", image.imageUrl);
                    const imageBuffer = fs.readFileSync(imagePath);
                    return {
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const { product_id, productName, productDescription, category, price, discountPrice, shelfTime, stock, expiryDate, manufactureDate, ingredients } = req.body;
        ...
        if (updatedProduct?.images) {
            productImagesBase64 = await Promise.all(updatedProduct.images.map(async (image) => {
                try {
                    const imagePath = path.join(process.cwd(), "uploads", image.imageUrl);
                    const imageBuffer = fs.readFileSync(imagePath);
                    return {
    ...
    try {
        ...
        const allProducts = await Promise.all(products.map(async (p) => {
            ...
            if (p.images && p.images.length > 0) {
                productImagesBase64 = await Promise.all(p.images.map(async (image) => {
                    try {
                        const imagePath = path.join(process.cwd(), "uploads", image.imageUrl);
                        const imageBuffer = fs.readFileSync(imagePath);
                        return {
...
export const fetchSingleProduct = async (req, res) => {
    try {
        ...
        if (product.images && product.images.length > 0) {
            productImagesBase64 = await Promise.all(product.images.map(async (image) => {
                try {
                    const imagePath = path.join(process.cwd(), "uploads", image.imageUrl);
                    const imageBuffer = fs.readFileSync(imagePath);
                    return {
...
// 1. Add Mystery Box
export const addMysteryBox = async (req, res) => {
    try {
        const sellerId = req.user.seller_id;
        console.log("sellerid: ", sellerId);
        const { name, description, price, discountPrice, category, stock, productDetails, // JSON string of [{productId: 1, quantity: 2}, {productId: 3, quantity: 1}]
        totalValue, expiryDate, // Add this
...
export const fetchMysteryBoxes = async (req, res) => {
    try {
        const { category, minPrice, maxPrice, sellerId, sortBy = 'newest', page = 1, limit = 20 } = req.query;
        ...
        const updatedMysteryBoxes = await Promise.all(mysteryBoxes.map(async (box) => {
            let storeImgBase64 = null;
...
export const fetchSellerMysteryBoxes = async (req, res) => {
    try {
        const sellerId = req.user.seller_id;
        if (!sellerId) {
            return res.status(401).json({ message: "Unauthorized access" });
        ...
        const mysteryBoxesWithProducts = await Promise.all(mysteryBoxes.map(async (box) => {
            try {
                ...
                const enrichedProducts = productsArray.map((item) => {
                ...
                return {
                    ...
                    totalItems: productsArray.reduce((sum, item) => sum + item.quantity, 0)
                };
            }
            catch (error) {
...
export const fetchSingleMysteryBox = async (req, res) => {
    try {
        ...
        if (sellerId && mysteryBox.sellerId === sellerId) {
            try {
                ...
                const enrichedProducts = await Promise.all(productsArray.map(async (item) => {
                    const product = products.find(p => p.product_id === item.productId);
                    if (product) {
                        // Convert images to base64
                        const productImages = await Promise.all(product.images.map(async (image) => {
                            try {
                                const imagePath = path.join(process.cwd(), "uploads", image.imageUrl);
                                const imageBuffer = fs.readFileSync(imagePath);
                                return {
                ...
                response.totalItems = productsArray.reduce((sum, item) => sum + item.quantity, 0);
            }
            catch (error) {
                console.error("Error parsing productDetails:", error);
                response.products = [];
...
// 5. Update Mystery Box
export const updateMysteryBox = async (req, res) => {
    try {
        const sellerId = req.user.seller_id;
        if (!sellerId) {
            return res.status(401).json({ message: "Please login again." });
        }
        const { id, name, description, price, discountPrice, category, stock, status, productDetails, totalValue, expiryDate, // Add this
        manufactureDate // Add this
         } = req.body;
        ...
        if (expiryDate !== undefined)
            updateData.expireDate = expiryDate ? new Date(expiryDate) : null; // Add this
        if (manufactureDate !== undefined)
            updateData.manufactureDate = manufactureDate ? new Date(manufactureDate) : null; // Add this
...
export const deleteMysteryBox = async (req, res) => {
    try {
        const sellerId = req.user.seller_id;
        if (!sellerId) {
            return res.status(401).json({ message: "Please login again." });
...
export const getMysteryBoxStats = async (req, res) => {
    try {
        const sellerId = req.user.seller_id;
        if (!sellerId) {
            return res.status(401).json({ message: "Unauthorized access" });
...
export const getMysteryBoxesBySeller = async (req, res) => {
    try {
