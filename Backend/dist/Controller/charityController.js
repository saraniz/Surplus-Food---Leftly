...
export const registerCharity = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: newCharity.id, role: "charity", email: newCharity.email }, process.env.JWT_SECRET, { expiresIn: "1h" });
...
export const getCharityDetail = async (req, res) => {
    if (!req.user) {
...
export const updateCharityProfile = async (req, res) => {
    try {
        const charityId = req.user.id;
