...
export const placeOrder = async (req, res) => {
    try {
        const { deliveryAddress, deliveryInfo, deliveryTime, deliveryFee, totalPrice, paymentMethod, items, guestEmail, guestName, guestPhone, isGuest = false, } = req.body;
            ...
            for (const item of items) {
                ...
                if (isMysteryBox) {
                    ...
                    try {
                        ...
                        const totalItems = productDetails.reduce((sum, p) => sum + p.quantity, 0);
                    ...
                    if (product.stock < item.quantity) {
                        throw new Error(`${product.productName} is out of stock or insufficient quantity`);
                    }
                    // 2. Insert order item
                    await tx.orderItem.create({
                        data: {
