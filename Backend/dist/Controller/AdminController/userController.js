...
const calculateCustomerTotalOrders = async (customerId) => {
    return await prisma.order.count({
        where: { customerId: customerId }
    });
};
// Helper function to get customer's last activity
const getCustomerLastActivity = async (customerId) => {
...
export const getAllSellers = async (req, res) => {
    try {
        ...
        const sellersWithStats = await Promise.all(allsellers.map(async (seller) => {
...
export const getCustomerDetails = async (req, res) => {
    try {
        ...
        const totalSpent = orders.reduce((total, order) => total + order.totalAmount + order.deliveryFee, 0);
...
async function getGrowthData(userType) {
