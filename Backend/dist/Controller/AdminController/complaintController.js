...
const getComplainantDetails = async (type, id) => {
    if (type === 'CUSTOMER') {
...
const getAccusedDetails = async (type, id) => {
    if (type === 'CUSTOMER') {
        return await prisma.customer.findUnique({
            where: { id },
            select: {
...
const recordStatusChange = async (complaintId, oldStatus, newStatus, adminId, note) => {
...
export const getAllComplaints = async (req, res) => {
    try {
        ...
        const { page = '1', limit = '10', status, priority, type, complainantType, search, startDate, endDate } = req.query;
        if (!admin) {
        ...
        const enrichedComplaints = await Promise.all(complaints.map(async (complaint) => {
            const complainant = await getComplainantDetails(complaint.complainantType, complaint.complainantId);
            const accused = await getAccusedDetails(complaint.accusedType, complaint.accusedId);
            return {
                ...complaint,
                complainant,
                accused,
...
export const createComplaint = async (req, res) => {
    try {
        const { title, description, complaintType, priority, complainantType, complainantId, accusedType, accusedId, orderId, } = req.body;
...
export const updateComplaint = async (req, res) => {
    try {
        ...
        if (status && status !== complaint.status) {
            await recordStatusChange(parseInt(id), complaint.status, status, admin.id, note || "Status updated");
        }
...
export const resolveComplaint = async (req, res) => {
    try {
        ...
        await recordStatusChange(parseInt(id), complaint.status, status || 'RESOLVED', admin.id, `Complaint resolved: ${resolution.substring(0, 100)}...`);
        return res.status(200).json({
            complaint: updatedComplaint,
...
export const getComplaintAnalytics = async (req, res) => {
    try {
        ...
        if (type === 'types') {
            ...
            const resolvedWithTime = complaints.filter(c => (c.status === 'RESOLVED' || c.status === 'CLOSED') && c.resolvedAt);
