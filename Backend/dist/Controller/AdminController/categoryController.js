...
export const getProductsByCategoryWithFilters = async (req, res) => {
    try {
        const { categoryName } = req.params;
        const { minPrice, maxPrice, sortBy = 'sales', // sales, price_asc, price_desc, rating, newest
...
export const getCategoryProductsStats = async (req, res) => {
    try {
        const { categoryName } = req.params;
        if (!categoryName) {
            return res.status(400).json({ message: "Category name is required" });
